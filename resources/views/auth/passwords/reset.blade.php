...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-blue-100 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        ...
        <div class="bg-white rounded-xl shadow-lg p-8">
            ...
            <form id="reset-form" class="space-y-6" action="{{ route('password.update') }}" method="POST">
                @csrf
                ...
                <div>
                    ...
                    <div class="mt-1 relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <i class="fas fa-lock text-gray-400" id="reset-lock-icon"></i>
                        </div>
                        <input id="password" name="password" type="password" required autofocus class="appearance-none block w-full pl-10 pr-10 py-3 border border-gray-300 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-300 @error('password') border-red-500 @enderror" placeholder="Minimal 8 karakter">
                    ...
                    <div id="reset-password-checklist" class="mt-2 space-y-1.5 hidden" style="transition: all 0.3s ease;">
                        <div class="flex items-center text-sm">
                            <i class="fas fa-circle text-gray-300 mr-2 text-[10px] transition-all duration-300" id="reset-icon-length"></i>
                            <span class="text-gray-500 transition-colors duration-300" id="reset-text-length">Minimal 8 karakter</span>
                        </div>
                        <div class="flex items-center text-sm">
                            <i class="fas fa-circle text-gray-300 mr-2 text-[10px] transition-all duration-300" id="reset-icon-uppercase"></i>
                            <span class="text-gray-500 transition-colors duration-300" id="reset-text-uppercase">Minimal 1 huruf besar (A-Z)</span>
                        </div>
                        <div class="flex items-center text-sm">
                            <i class="fas fa-circle text-gray-300 mr-2 text-[10px] transition-all duration-300" id="reset-icon-lowercase"></i>
                            <span class="text-gray-500 transition-colors duration-300" id="reset-text-lowercase">Minimal 1 huruf kecil (a-z)</span>
                        </div>
                        <div class="flex items-center text-sm">
                            <i class="fas fa-circle text-gray-300 mr-2 text-[10px] transition-all duration-300" id="reset-icon-number"></i>
                            <span class="text-gray-500 transition-colors duration-300" id="reset-text-number">Minimal 1 angka (0-9)</span>
                        </div>
                    </div>
                    @error('password')
                ...
                <div>
                    ...
                    <div class="mt-1 relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <i class="fas fa-lock text-gray-400" id="reset-lock-icon-conf"></i>
                        </div>
                    ...
                    <div id="reset-confirm-message" class="mt-2 hidden" style="transition: all 0.3s ease;">
                        <div class="flex items-center text-sm">
                            <i class="fas fa-circle text-gray-300 mr-2 text-[10px] transition-all duration-300" id="reset-icon-match"></i>
                            <span class="text-gray-500 transition-colors duration-300" id="reset-text-match">Password harus sama</span>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    // Real-time password validation for reset page
    document.addEventListener('DOMContentLoaded', function() {
        const resetForm = document.getElementById('reset-form');
        const passwordInput = document.getElementById('password');
        const confirmInput = document.getElementById('password_confirmation');
        const checklist = document.getElementById('reset-password-checklist');
        const lockIcon = document.getElementById('reset-lock-icon');
        const lockIconConf = document.getElementById('reset-lock-icon-conf');
        ...
        passwordInput.addEventListener('input', function() {
            ...
            rules.forEach(rule => {
                const icon = document.getElementById('reset-icon-' + rule.id);
                const text = document.getElementById('reset-text-' + rule.id);
        ...
        function validateConfirmation() {
            ...
            const matchIcon = document.getElementById('reset-icon-match');
            const matchText = document.getElementById('reset-text-match');
        ...
        resetForm.addEventListener('submit', function(e) {
            ...
            if (errors.length > 0) {
                ...
                showToast('Reset Password Gagal', 'Password baru belum memenuhi syarat:' + listHtml, 'error');
                return false;
            }
            ...
            if (val !== confVal) {
                e.preventDefault();
                confirmInput.focus();
                showToast('Konfirmasi Tidak Cocok', 'Password baru dan konfirmasi harus sama.', 'warning');
                return false;
            }
        });
    });
