...
<div class="relative bg-gradient-to-r from-blue-600 to-blue-500 py-16">
    <div class="container mx-auto px-4 relative z-10">
        <div class="text-center text-white">
            <h1 class="text-4xl md:text-5xl font-bold mb-4">{{ $category->name ?? 'Sewa Pancing' }}</h1>
            <p class="text-xl opacity-90">{{ $category->description ?? 'Layanan penyewaan alat pancing air tawar' }}</p>
        </div>
    </div>
</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow-lg p-6 sticky top-24">
                ...
                <form action="{{ route('produk.sewa-pancing') }}" method="GET" class="space-y-4">
                    <div>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        @if(request()->anyFilled(['search', 'subcategory', 'min_price', 'max_price']))
                            <a href="{{ route('produk.sewa-pancing') }}" class="w-full bg-gray-200 text-gray-700 py-2 rounded-lg hover:bg-gray-300 transition text-center text-sm font-medium">
                                Reset Filter
                            </a>
                        @endif
                    </div>
