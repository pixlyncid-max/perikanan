        ...
        <form method="POST" action="{{ route('admin.news.update', $news) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                @error('featured_image')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
                <div id="image-preview" class="mt-2 {{ $news->featured_image ? '' : 'hidden' }}">
                    <img src="{{ $news->featured_image ? asset('storage/' . $news->featured_image) : '' }}" alt="Preview" class="h-32 w-32 object-cover rounded-lg">
                </div>
            </div>
            ...
            <!-- Published At -->
            <div id="published-at-container" class="{{ old('is_published', $news->is_published) ? '' : 'hidden' }}">
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                    ...
                    <div>
                        <span class="font-medium">Terakhir Diperbarui:</span> {{ $news->updated_at->format('d M Y H:i') }}
                    </div>
                </div>
            </div>
